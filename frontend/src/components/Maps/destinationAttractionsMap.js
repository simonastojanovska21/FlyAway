...
const DestinationAttractionsMap=(props)=>{
        ...
        <div>
            <Modal
                ...
                <Modal.Body className={"border-0 rounded-0"}>
                    <MapContainer center={coordinates} zoom={10} id={"tripMap"}>
                        <TileLayer
                        ...
                        {props.items.map((each)=>{
                            return(
                                <Marker position={[each.latitude, each.longitude]}>
                                    <Popup>
                                        <h5>{each.name}</h5>
...
export default DestinationAttractionsMap;